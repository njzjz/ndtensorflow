...
def irfftn(
    ...
    *,
    ...
    out = _apply_1d(
        out,
        lambda y: tf.signal.irfft(y, fft_length=[sizes[-1]] if sizes[-1] is not None else None),
        None,
        axes_[-1],
    )
    return _wrap(_scale_inverse(out, n, norm))
...
def hfft(
    ...
    *,
    ...
    return _wrap(_unwrap(irfft(_wrap(tf.math.conj(tensor)), n=size, axis=axis, norm=norm)) * tf.cast(size, _real_dtype(tensor.dtype)))
...
def ihfft(
    ...
    *,
    ...
    tensor = _unwrap(x)
    size = n if n is not None else tensor.shape[axis]
    return _wrap(tf.math.conj(_unwrap(rfft(x, n=size, axis=axis, norm=norm))) / tf.cast(size, _complex_dtype(tensor.dtype)))
...
def fftshift(
    ...
    *,
    ...
    tensor = _unwrap(x)
    axes_ = _normalize_axes(None if axes is None else (axes if isinstance(axes, Sequence) else (axes,)), tensor.shape.rank, None)

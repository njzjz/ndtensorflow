...
def _logical_two_arg(f):
    ...
    def _f(x1: Any, x2: Any, /, **kwargs: Any) -> Array:
        return Array._from_tensor(
            f(
                tf.convert_to_tensor(_unwrap(x1), dtype=tf.bool),
                tf.convert_to_tensor(_unwrap(x2), dtype=tf.bool),
                **kwargs,
            )
        )
...
def _moveaxis(
    ...
    tensor = _unwrap(x)
    return tf.transpose(tensor, _moveaxis_permutation(tensor.shape.rank, source, destination))
...
def expm1(x: Array, /) -> Array:
    ...
    if _is_complex(tensor.dtype):
        ...
        out = tf.where(plus_inf_real, tf.where(imag_part == 0, zero_imag_out, exp_out), out)
        ...
        out = tf.where(minus_inf_real, tf.complex(-tf.ones_like(real_part), minus_inf_imag), out)
...
def trunc(x: Array, /) -> Array:
    ...
    if _is_integer(tensor.dtype):
        return Array._from_tensor(tf.identity(tensor))
    return Array._from_tensor(tf.where(tensor < 0, tf.math.ceil(tensor), tf.math.floor(tensor)))
...
def round(x: Array, /, *, decimals: int = 0) -> Array:
    ...
    if tensor.dtype in _integral_dtypes:
        return Array._from_tensor(tf.identity(tensor))
    if _is_complex(tensor.dtype):
        real_part = round(Array._from_tensor(tf.math.real(tensor)), decimals=decimals).unwrap()
        imag_part = round(Array._from_tensor(tf.math.imag(tensor)), decimals=decimals).unwrap()
...
def sign(x: Array, /) -> Array:
    ...
    if tensor.dtype in _unsigned_dtypes:
        return Array._from_tensor(
            tf.where(tensor == 0, tf.zeros((), dtype=tensor.dtype), tf.ones((), dtype=tensor.dtype))
        )
...
def signbit(x: Array, /) -> Array:
    ...
    if tensor.dtype == tf.float16:
        return Array._from_tensor(tf.bitwise.right_shift(tf.bitcast(tensor, tf.uint16), 15) == 1)
    if tensor.dtype == tf.bfloat16:
        return Array._from_tensor(tf.bitwise.right_shift(tf.bitcast(tensor, tf.uint16), 15) == 1)
    if tensor.dtype == tf.float32:
        return Array._from_tensor(tf.bitwise.right_shift(tf.bitcast(tensor, tf.uint32), 31) == 1)
    if tensor.dtype == tf.float64:
        return Array._from_tensor(tf.bitwise.right_shift(tf.bitcast(tensor, tf.uint64), 63) == 1)
...
def _as_bool(x: Array) -> tf.Tensor:
    tensor = _unwrap(x)
    return tensor if tensor.dtype == tf.bool else tensor != tf.zeros((), dtype=tensor.dtype)
...
def prod(
    ...
    *,
    ...
    return Array._from_tensor(tf.reduce_prod(tf.cast(tensor, dtype), axis=axis, keepdims=keepdims))
...
def sum(
    ...
    *,
    ...
    tensor = _unwrap(x)
    dtype = dtype or _accumulation_dtype(tensor.dtype)
    return Array._from_tensor(tf.reduce_sum(tf.cast(tensor, dtype), axis=axis, keepdims=keepdims))
...
def std(
    ...
    *,
    ...
    return sqrt(var(x, axis=axis, correction=correction, dtype=dtype, keepdims=keepdims))
...
def stack(arrays: tuple[Array, ...] | list[Array], /, *, axis: int = 0) -> Array:
    dtype = result_type(*arrays)
    return Array._from_tensor(tf.stack([_to_tensor(x, dtype) for x in arrays], axis=axis))
...
def nonzero(x: Array, /) -> tuple[Array, ...]:
    ...
    return tuple(Array._from_tensor(item) for item in tf.unstack(tf.transpose(indices), axis=0))
...
def _unique(x: Array) -> tuple[tf.Tensor, tf.Tensor, tf.Tensor, tf.Tensor]:
    ...
    inverse = tf.argmax(tf.cast(inverse_equality, tf.int64), axis=1, output_type=tf.int64)

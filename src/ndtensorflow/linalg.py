...
def pinv(x: Array, /, *, rtol: float | Array | None = None) -> Array:
    ...
    return _wrap(tf.linalg.matmul(v_scaled, tf.linalg.matrix_transpose(tf.math.conj(u))))
...
def vector_norm(
    ...
    *,
    ...
    if ord == 0:
        out = tf.cast(count_nonzero(Array._from_tensor(x_), axis=axis_).unwrap(), out_dtype)
    elif ord == 1:
